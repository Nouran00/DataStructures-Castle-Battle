...
void GUI::PrintMessage(string msg) const	//Prints a message on status bar
{
	...
	std::string* SingleLine = new string[count];
	temp = msg;
	std::stringstream iss2(temp);
	while (iss2.good())
	{
		getline(iss2, SingleLine[c], '\n');
		c++;
	}
	int totsize = (count)*20;
	int mid = WindHeight - (int)(StatusBarHeight / (1.8));
	int top = mid - totsize / 2;
	for (int i = 0;i < count;i++)
	{
		pWind->DrawString(10, (int)(top + 20*i ), SingleLine[i]);
	}
